...
namespace Query4 {
	...
	struct simpleHugeBatchBfs {
		...
		typedef BatchBitsAMA<bit_t, width> Bitset;//表示一个顶点的bfs情况
		...
		static void runBatch(std::vector<BatchBFSdata>& bfsData, const PersonSubgraph& subgraph
			...
			for (int a = 0; a < subgraphSize; a++) {
				const auto ret = posix_memalign(reinterpret_cast<void**>(&(verstatus[a])), 64, sizeof(BatchBitsAMA<BITYPE, BITYPE_WIDTH>)*numQueries);//数据对齐（分配内存首地址，对齐边界，指定分配字节大小）
				if (unlikely(ret != 0)) {
			...
			for (int a = 0; a < subgraphSize; a++) {
				const auto ret = posix_memalign(reinterpret_cast<void**>(&(next_verstatus[a])), 64, sizeof(BatchBitsAMA<BITYPE, BITYPE_WIDTH>)*numQueries);//数据对齐（分配内存首地址，对齐边界，指定分配字节大小）
				if (unlikely(ret != 0)) {
					std::cout << "unlikely" << std::endl;
					throw - 1;
				}
			...
			alignas(64) uint32_t numDistDiscovered[BATCH_BITS_COUNT][SAMPLE_NUMS];//512，numDistDiscovered每一个数据代表该bfs在一轮搜索中搜索到的新的顶点数
		...
		static bool __attribute__((hot)) runBatchRound(const PersonSubgraph& subgraph, const PersonId startPerson,
			const PersonId limit, Bitset* visitList, Bitset* nextVisitList, BatchDistance<BITYPE, BITYPE_WIDTH>* batchDist,
			...
			for (PersonId curPerson = startPerson; curPerson < limit; ++curPerson) {
				...
				while (friendsBounds.first != friendsBounds.second) {
					...
					for (int i = 0; i < width; i++) {
						bit_t cover = curVisit.data[i] & curFriend.data[i];
						bit_t cover1 = cover;
							...
							for (int j = 0; j < TYPE_BITS; j++) {
								if (BitBaseOp<bit_t>::notZero(cover & BitBaseOp<bit_t>::getSetMask(j))) {
									...
									for (int s = 0; s < BITYPE_WIDTH; s++) {
										D = verstatus[curPerson][i*TYPE_BITS + j].data[s] & friends.data[s] & ~verstatus[*friendsBounds.first][i*TYPE_BITS + j].data[s];
										if (BitBaseOp<BITYPE>::notZero(D)) {
											...
											insert = true;
											//numofone += BitBaseOp<BITYPE>::popCount(D);
											//bfsData[i*TYPE_BITS + j].totalReachable += numofone;
										}
									}
									//bfsData[i*TYPE_BITS + j].totalDistances += numofone * nextDistance;
									//bfsData[i*TYPE_BITS + j].totalReachable += numofone;
									//std::cout << nextDistance<<" , " << numofone << std::endl;
									if (insert) {
										seen[*friendsBounds.first].data[i] |= BitBaseOp<bit_t>::getSetMask(j);
									}
									else {
										cover1 &= ~BitBaseOp<bit_t>::getSetMask(j);
									}
								}
							}
							if (BitBaseOp<bit_t>::notZero(cover1)) {
								nextVisitList[*friendsBounds.first].data[i] |= cover1;
					...
					for (int i = 0; i < width; i++) {
						bit_t nocover = curVisit.data[i] & ~curFriend.data[i];
						bit_t nocover1 = nocover;
						if (BitBaseOp<bit_t>::notZero(nocover)) {
							/*unsigned pos = 0;
							while (nocover % 2 == 0 && pos < TYPE_BITS) {
								nocover = nocover >> 1;
								pos++;
							}
							while (pos < TYPE_BITS) {

								//std::cout << BitBaseOp<bit_t>::popCount(nocover) << " " << pos << std::endl;
								bool insert = false;
								for (int s = 0; s < BITYPE_WIDTH; s++) {

									const BITYPE D = ((verstatus[curPerson][i*TYPE_BITS + pos].data[s] & friends.data[s]) & ~(verstatus[*friendsBounds.first][i*TYPE_BITS + pos].data[s]));
									if (BitBaseOp<BITYPE>::notZero(D)) {
										insert = true;
										verstatus[*friendsBounds.first][i*TYPE_BITS + pos].data[s] |= D;
										//uint64_t numofone = BitBaseOp<BITYPE>::popCount(D);
										//bfsData[i*TYPE_BITS + j].totalDistances += numofone * nextDistance;
										//bfsData[i*TYPE_BITS + j].totalReachable += numofone;
									}
								}
								if (!insert) {
									nocover1 &= ~BitBaseOp<bit_t>::getSetMask(pos);
								}
								nocover = nocover >> 1;
								pos++;
								while (nocover % 2 == 0 && pos < TYPE_BITS) {
								...
								if (BitBaseOp<bit_t>::notZero(nocover & BitBaseOp<bit_t>::getSetMask(j))) {
									...
									for (int s = 0; s < BITYPE_WIDTH; s++) {
										...
										if (BitBaseOp<BITYPE>::notZero(D)) {
											...
											//uint64_t numofone = BitBaseOp<BITYPE>::popCount(D);
											batchDist[i*TYPE_BITS + j].updateDiscovered(D, s);
										}
									}
									if (!insert) {
						...
						for (int j = 0; j < TYPE_BITS; j++) {
							if (BitBaseOp<bit_t>::notZero(seenele & BitBaseOp<bit_t>::getSetMask(j))) {
								...
								for (int l = 0; l < BITYPE_WIDTH; l++) {
									batchDist[i*TYPE_BITS + j].updateDiscovered(next_verstatus[k][i*TYPE_BITS + j].data[l], l);
