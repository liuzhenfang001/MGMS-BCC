...
struct BatchDistance<uint64_t, width> {
   ...
   BatchDistance() :numDistDiscovered(nullptr), localDiscoveredIteration() {
	   memset(localDiscovered, 0, sizeof(uint64_t) * 8 * width);
	   memset(localDiscoveredIteration, 0, width);
   }
...
struct BatchDistance<__m256i, width> {
   ...
   void updateDiscoveredNodes(const uint64_t partIx) {
      for(unsigned a=0; a<8; a++) {
		 ...
		 //std::cout<< numDistDiscoveredBase[8 * 20] <<" ";
      }
	  //for (int i = 0; i < SAMPLE_NUMS; i++) {
		//  std::cout << numDistDiscovered[i] << " ";
	  //}
	  //std::cout <<std::endl;
   ...
   void updateDiscovered(__m256i newVisits, uint64_t partIx) {
	  ...
	  //__m256i c = _mm256_set_epi32(0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000001, 0x00000000);
	  //int T = _mm256_testc_si256(_mm256_setzero_si256(), localDiscoveredBase[7] & c) != 0 ? 1 : 0;
	  //cout << T <<" ";
      if(localDiscoveredIteration[partIx]==254) {//·ÀÖ¹Òç³ö
         updateDiscoveredNodes(partIx);
         localDiscoveredIteration[partIx]=0;
      } else {
         localDiscoveredIteration[partIx]++;
